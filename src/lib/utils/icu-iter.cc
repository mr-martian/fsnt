 ...
 * https://github.com/apertium/lexd/blob/master/src/icu-iter.cc
 */
#include "icu-iter.h"
#include <iostream>
#include <string>
...
int
stoi(const UnicodeString& s)
{
  return stoi(to_wstring(s));
}

unsigned long
stoul(const UnicodeString& s)
{
  return stoul(to_wstring(s));
}

double
stod(const UnicodeString& s)
{
  return stod(to_wstring(s));
}
